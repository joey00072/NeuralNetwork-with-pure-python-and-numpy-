...
class NeuralNetwork(object):
    ...
    def init_weight(self, input, output):
        ...
        arr = [[random.randint(-8, 8)/10 for j in range(input)]
    ...
    def get_biases(self, layers):
        ...
        for i in range(1, len(layers)):
            b = np.matrix(
                [random.randint(-8, 8)/10 for j in range(layers[i])]).T
            biases.append(b)
        return biases
    ...
    def backProp(self, inputs, y):
        ...
        for i in range(n):
            ...
            # gredients
            dEdA = errors[i]
            # sigmoid_prime (derivative)
            dAdZ = np.multiply(layers_tensors_r[i], (1-layers_tensors_r[i]))
...
if __name__ == '__main__':
    ...
    print("## predction without training for xor input=[1,1]")
    ...
    print("##")
    print("## predction with training for xor input=[1,1]")
    print(brain.predict([1, 1]))
    # print(sum(brain.error([0,0],[0])))

    print("Enter 2 number with space for prediction")
    print("Enter t to train")
